...
run_paramo_recode <- function(i, N) {
  ...
  # Recode paramo's automatic recoding to remove ambiguity at tips 
  td.comb$dat$`leaf_under+bulb`[which( td.comb$dat$`leaf_under+bulb` == "0&2")] <- "0"
  td.comb$dat$`stem_under+corm+rhizome`[which(td.comb$dat$`stem_under+corm+rhizome` == "0&4")] <- "0"
  td.comb$dat$`root_tuber+disc_tub+cont_tub`[which(td.comb$dat$`root_tuber+disc_tub+cont_tub` == "0&4")] <- "0"  
